...
int csSetSurfEdSurface( char *surfname, char *objname )
{
   ModData *md = getModData();
   char cmd[ 256 ];
...
int csRenameSurface( char *newname )
{
   ModData *md = getModData();
   char cmd[ 128 ];
...
int csSetSurfaceInt( char *ch, int val )
{
   ModData *md = getModData();
   char cmd[ 128 ];
...
int csSetSurfaceFlt( char *ch, double val )
{
   ModData *md = getModData();
   char cmd[ 128 ];
...
int csSetSurfaceColor( char *ch, double color[ 3 ] )
{
   ModData *md = getModData();
   char cmd[ 128 ];
...
int csAddSurfaceShader( char *shader )
{
   ModData *md = getModData();
   char cmd[ 128 ];
...
int csRemSurfaceShader( char *shader )
{
   ModData *md = getModData();
   char cmd[ 128 ];
...
int csCopySurface( LWSurfaceID to, LWSurfaceID from )
{
   ...
   sprintf( cmd, "Surf_Copy %s %s", to, from );
   md->cmderror = md->local->evaluate( md->local->data, cmd );
   return !md->cmderror;
}
