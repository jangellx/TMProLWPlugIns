/*
 * hybridmfractal.c --  Layout Procedural Texture Plugin
 *
...
HybridMFractalCreate (void *priv, LWTLayerID txtrLayer, LWError *err)
{
  HybridMFractal	*inst = NULL;
  LWChanGroupID		 parent = NULL;


  // Allocate instance data.
  inst = malloc (sizeof (HybridMFractal));
  if (!inst) {
    *err = "No memory for new HybridMFractal texture.";
    return NULL;
  }

  // Initialize the current version number
  inst->tversion = HM_VERSION;
  ...
  inst->off[0] = 0.35; inst->off[1] = 0.0; inst->off[2] = 0.0;
  ...
  if (inst->increment = (*vpfunc->create) (LWVP_FLOAT, LWVPDT_NOTXTR)) {
    inst->incgrp = (*evfunc->createGroup) (parent, "HM_Increment");
  ...
  if (inst->lacunarity = (*vpfunc->create) (LWVP_FLOAT, LWVPDT_NOTXTR)) {
    inst->lacgrp = (*evfunc->createGroup) (parent, "HM_Lacunarity");
  ...
  if (inst->octaves = (*vpfunc->create) (LWVP_FLOAT, LWVPDT_NOTXTR)) {
    inst->octgrp = (*evfunc->createGroup) (parent, "HM_Octaves");
  ...
  if (inst->offset = (*vpfunc->create) (LWVP_FLOAT, LWVPDT_NOTXTR)) {
    inst->offgrp = (*evfunc->createGroup) (parent, "HM_Offset");
 ...
 */
XCALL_(static void)
HybridMFractalDestroy (HybridMFractal *inst)
{
  if (inst) {
...
HybridMFractalLoad (HybridMFractal *inst, const LWLoadState *lState)
{
  LWError     err;
  short	      fn;
 ...
 */
XCALL_(static LWError)
HybridMFractalSave (HybridMFractal *inst, const LWSaveState *sState)
{
  LWError    err;
  short      fn;
 ...
 */
XCALL_(static LWError)
HybridMFractalCopy (HybridMFractal *to, HybridMFractal *from)
{
  LWError	 err;
...
HybridMFractalDescLn (HybridMFractal *inst)
{
 ...
 */
XCALL_(static LWError)
HybridMFractalInit (HybridMFractal *inst, int rtype)
{
  return (NULL);
}
 ...
 */
XCALL_(static void)
HybridMFractalCleanup (HybridMFractal *inst)
{
  return;
}
 ...
 */
XCALL_(static LWError)
HybridMFractalNewTime (HybridMFractal *inst, LWFrame f, LWTime t)
{
...
HybridMFractalFlags (HybridMFractal *inst)
{
  return (LWTEXF_DISPLACE);
}
...
HybridMFractalEvaluate (HybridMFractal *inst, LWTextureAccess *ta)
{
  double     turb;
  double     PP[3];
  ...
  turb = HybridMultifractal(PP, inst->inc[0], inst->lac[0], inst->oct[0],
 ...
 */
XCALL_(int)
HybridMFractalActivate (long version, GlobalFunc *global, LWTextureHandler *local,
  void *serverData)
{
  ...
  local->inst->create   = HybridMFractalCreate;
  local->inst->destroy  = HybridMFractalDestroy;
  local->inst->load     = HybridMFractalLoad;
  local->inst->save     = HybridMFractalSave;
  local->inst->copy     = HybridMFractalCopy;
  local->inst->descln   = HybridMFractalDescLn;

  local->rend->init     = HybridMFractalInit;
  local->rend->cleanup  = HybridMFractalCleanup;
  local->rend->newTime  = HybridMFractalNewTime;

  local->flags    = HybridMFractalFlags;
  local->evaluate = HybridMFractalEvaluate;
 ...
 */
void *
HM_view_get ( HybridMFractal *inst, unsigned long vid ) 
{
  void *result = NULL;
 ...
 */
int
HM_view_set ( HybridMFractal *inst, unsigned long vid, void *value )
{
  int result = 0;
 ...
 */
void
HM_view_destroy ( void *myData )
{
  HybridMFractal	*inst = (HybridMFractal*)myData;
 ...
 */
int
HybridMFractalInterface (long version, GlobalFunc *global, LWInterface *iface,
  void *serverdata )
{
  int rc = AFUNC_OK;
  ...
  static LWXPanelHint HM_hint[] = {
    XpLABEL(0,"HybridMultiFractal"),
    XpDESTROYNOTIFY(HM_view_destroy),
    XpEND
  };
  ...
  inst = (HybridMFractal *) iface->inst;
  ...
  // we can destroy it and create another, or return the existing instance.
    ...
    if (panID) {
      ...
      (*xpfunc->hint) ( panID, 0, HM_hint );

      // Describe the structure of the data instance to the panel
      (*xpfunc->describe)( panID, OPTS4_data_descrip, HM_view_get, HM_view_set);
