/*
 * heteroterrain.c --  Layout Procedural Texture Plugin
 *
...
HeteroTerrainCreate (void *priv, LWTLayerID txtrLayer, LWError *err)
{
  ...
  inst = malloc (sizeof (HeteroTerrain));
  if (!inst) {
    *err = "No memory for new HeteroTerrain texture.";
    return NULL;
  }

  // Initialize the current version number
  inst->tversion = HT_VERSION;
  ...
  if (inst->increment = (*vpfunc->create) (LWVP_FLOAT, LWVPDT_NOTXTR)) {
    inst->incgrp = (*evfunc->createGroup) (parent, "HT_Increment");
  ...
  if (inst->lacunarity = (*vpfunc->create) (LWVP_FLOAT, LWVPDT_NOTXTR)) {
    inst->lacgrp = (*evfunc->createGroup) (parent, "HT_Lacunarity");
  ...
  if (inst->octaves = (*vpfunc->create) (LWVP_FLOAT, LWVPDT_NOTXTR)) {
    inst->octgrp = (*evfunc->createGroup) (parent, "HT_Octaves");
  ...
  if (inst->offset = (*vpfunc->create) (LWVP_FLOAT, LWVPDT_NOTXTR)) {
    inst->offgrp = (*evfunc->createGroup) (parent, "HT_Offset");
 ...
 */
XCALL_(static void)
HeteroTerrainDestroy (HeteroTerrain *inst)
{
  if (inst) {
...
HeteroTerrainLoad (HeteroTerrain *inst, const LWLoadState *lState)
{
  LWError     err;
  short	      fn;
 ...
 */
XCALL_(static LWError)
HeteroTerrainCopy (HeteroTerrain *to, HeteroTerrain *from)
{
  LWError	 err;
...
HeteroTerrainDescLn (HeteroTerrain *inst)
{
 ...
 */
XCALL_(static LWError)
HeteroTerrainInit (HeteroTerrain *inst, int rtype)
{
  return (NULL);
}
 ...
 */
XCALL_(static void)
HeteroTerrainCleanup (HeteroTerrain *inst)
{
  return;
}
 ...
 */
XCALL_(static LWError)
HeteroTerrainNewTime (HeteroTerrain *inst, LWFrame f, LWTime t)
{
...
HeteroTerrainFlags (HeteroTerrain *inst)
{
  return (LWTEXF_HV_SRF |LWTEXF_DISPLACE );
}
 ...
 */
XCALL_(int)
HeteroTerrainActivate (long version, GlobalFunc *global, LWTextureHandler *local,
  void *serverData)
{
  ...
  local->inst->create   = HeteroTerrainCreate;
  local->inst->destroy  = HeteroTerrainDestroy;
  local->inst->load     = HeteroTerrainLoad;
  local->inst->save     = HeteroTerrainSave;
  local->inst->copy     = HeteroTerrainCopy;
  local->inst->descln   = HeteroTerrainDescLn;

  local->rend->init     = HeteroTerrainInit;
  local->rend->cleanup  = HeteroTerrainCleanup;
  local->rend->newTime  = HeteroTerrainNewTime;

  local->flags    = HeteroTerrainFlags;
  local->evaluate = HeteroTerrainEvaluate;
 ...
 */
void *
HT_view_get ( HeteroTerrain *inst, unsigned long vid ) 
{
  void *result = NULL;
 ...
 */
int
HT_view_set ( HeteroTerrain *inst, unsigned long vid, void *value )
{
  int result = 0;
 ...
 */
void
HT_view_destroy ( void *myData )
{
  HeteroTerrain	*inst = (HeteroTerrain*)myData;
