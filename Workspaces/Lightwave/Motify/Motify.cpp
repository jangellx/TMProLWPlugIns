...
extern "C" {
  // User Names
  #ifdef _DEBUG
    ServerUserName Motify_Usernames[]   = { {"TM-P  Mot-ify (Debug)", LANGID_USENGLISH | SRVTAG_USERNAME }, { "Mot-ify (Debug)", LANGID_USENGLISH | SRVTAG_BUTTONNAME }, { (const char *)NULL } };
  #else
    ServerUserName Motify_Usernames[]   = { {"TM-P  Mot-ify",         LANGID_USENGLISH | SRVTAG_USERNAME }, { "Mot-ify",         LANGID_USENGLISH | SRVTAG_BUTTONNAME }, { (const char *)NULL } };
  #endif
...
XCALL_ (int) Motify_Activate( long version, GlobalFunc *global,
  ...
  if( mui.sel_type == -1 ) {
    (*mui.message->error)( "Mot-ify Error: An objets, bone, light or camera must be selected.", NULL );
    return AFUNC_OK;
  }
