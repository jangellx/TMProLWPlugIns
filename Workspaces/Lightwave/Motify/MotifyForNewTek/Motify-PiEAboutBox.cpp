...
// These values are required by the macros in lwrasters.h
...
LWAboutPanel::LWAboutPanel( LWPanelFuncs *_panel_funcs,
  ...
  DrawFuncs *df = panel_funcs->drawFuncs;
  ...
  for( unsigned long i=0; lines[i] != NULL; i++ ) {
    if( stricmp1( lines[i], LWABOUT_HDIV ) == 0 ) {        // HDIV:   Horizontal divider; +5 to line size
      height += metrics->textHeight + 5;
    } else if( strcmp( lines[i], LWABOUT_VTAB ) == 0 ) {   // VTAB:   Just make an empty line
      height += metrics->textHeight;
    } else if( strcmp( lines[i], LWABOUT_YELLOW ) == 0 ) { // YELLOW: Doesn't affect panel height
      continue;
    } else if( strcmp( lines[i], LWABOUT_BLACK ) == 0 ) {  // BLACK:  Doesn't affect panel height
      continue;
    } else {
      // Text
...
void DrawAboutPanel( LWPanelID panel, LWAboutPanel *data, DrMode mode ) {
  DrawFuncs *df = data->panel_funcs->drawFuncs;
  const LWDisplayMetrics * metrics = df->dispMetrics();
  ...
  for( unsigned long i=0; data->lines[i] != NULL; i++ ) {
    if( strcmp( data->lines[i], LWABOUT_HDIV ) == 0 ) {          // HDIV
      PanelDraw::HorizontalDivider( data->panel, df, text_left + 10, line_pos + 8, text_width - 20, 1 );
      line_pos += line_height + 5;
    } else if( strcmp( data->lines[i], LWABOUT_VTAB ) == 0 ) {   // VTAB:  Just make an empty line
      line_pos += line_height;
    } else if( strcmp( data->lines[i], LWABOUT_YELLOW ) == 0 ) { // YELLOW
      color = COLOR_LT_YELLOW;
    } else if( strcmp( data->lines[i], LWABOUT_BLACK ) == 0 ) {  // BLACK
      color = COLOR_BLACK;
    } else {                                                     // Text
      line_width = df->textWidth( data->panel, (char *)data->lines[i] );

      df->drawText( data->panel, (char *)data->lines[i], color,
