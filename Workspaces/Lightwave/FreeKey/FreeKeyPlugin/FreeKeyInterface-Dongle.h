...
#include "../FreeKeyLicenseGen/FreeKeyLicense.h"
...
//   this returns true.
inline bool FreeKeyDongle::TestKey( const char * key, long dongle_id ) {
  if( key == NULL )
    return false;

  if( strcmp( GenerateFreeKeyLicense( dongle_id ), key ) == 0 )
    return true;
