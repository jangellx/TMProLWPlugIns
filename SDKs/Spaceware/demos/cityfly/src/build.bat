...
REM   $Id: build.bat,v 1.6 1996/11/13 21:01:25 prichard Exp $
...
echo Preparing to build CITYFLY.EXE
...
nmake /c /f "cityfly.mak" CFG="cityfly - Win32 %ARCH% Debug" CLEAN
:cleanr
if "%RELEASE%" == "False" goto build
echo Cleaning: ..\bin\%ARCH%\Release
nmake /c /f "cityfly.mak" CFG="cityfly - Win32 %ARCH% Release" CLEAN
goto end
...
nmake /c /f "cityfly.mak" CFG="cityfly - Win32 %ARCH% Debug" ALL
:buildr
if "%RELEASE%" == "False" goto end
echo Building: %ARCHC% Release
nmake /c /f "cityfly.mak" CFG="cityfly - Win32 %ARCH% Release" ALL
