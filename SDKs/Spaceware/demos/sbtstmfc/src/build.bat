...
REM   $Id: build.bat,v 1.1.2.3 1998/05/19 10:56:44 equerze Exp $
...
echo Preparing to build sbtstmfc.EXE
...
nmake /c /f "sbtstmfc.mak" CFG="sbtstmfc - Win32 %ARCHC% Debug" CLEAN
:cleanr
if "%RELEASE%" == "False" goto build
echo Cleaning: ..\bin\%ARCH%\Release
nmake /c /f "sbtstmfc.mak" CFG="sbtstmfc - Win32 %ARCHC% Release" CLEAN
goto end
...
nmake /c /f "sbtstmfc.mak" CFG="sbtstmfc - Win32 %ARCHC% Debug" ALL
:buildr
if "%RELEASE%" == "False" goto end
echo Building: %ARCHC% Release
nmake /c /f "sbtstmfc.mak" CFG="sbtstmfc - Win32 %ARCHC% Release" ALL
