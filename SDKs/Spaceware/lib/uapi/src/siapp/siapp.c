 ...
 *
 * $Id: siapp.c,v 1.8.2.1.2.1 1998/05/26 17:59:52 equerze Exp $
 *
...
static char cvsId[]="(C) 1998 Spacetec IMC Corporation: $Id: siapp.c,v 1.8.2.1.2.1 1998/05/26 17:59:52 equerze Exp $";
...
#ifdef _DEBUG
static LPCTSTR DllLibrary = "siappdld";
#else
...
{
   enum InitResult nResult;
   enum ErrorCode nErrorCode;
   ...
   if ((ghDll = LoadLibrary(DllLibrary)) != NULL)
      ...
      if ((pDllSpwRetVal = (enum SpwRetVal *) GetProcAddress(ghDll, strDLLRetVal)) == NULL)
         {
         nResult = FAILED;
      ...
      if ((pfnSiBeep = (PFNSI_BEEP) GetProcAddress(ghDll, fnSiBeep)) == NULL)
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }

      if ((pfnSiInit = (PFNSI_INIT) GetProcAddress(ghDll, fnSiInitialize)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiTerminate = (PFNSI_TERMINATE) GetProcAddress(ghDll, fnSiTerminate)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiGetNumDevices = (PFNSI_GETNUMDEVICES) GetProcAddress(ghDll, fnSiGetNumDevices)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
      
      if ((pfnSiGetDeviceID = (PFNSI_GETDEVICEID) GetProcAddress(ghDll, fnSiGetDeviceID)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }

      if ((pfnSiDeviceIndex = (PFNSI_DEVICEINDEX) GetProcAddress(ghDll, fnSiDeviceIndex)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiDispatch = (PFNSI_DISPATCH) GetProcAddress(ghDll, fnSiDispatch)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiOpenWinInit = (PFNSI_OPENWININIT) GetProcAddress(ghDll, fnSiOpenWinInit)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiOpen = (PFNSI_OPEN) GetProcAddress(ghDll, fnSiOpen)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiClose = (PFNSI_CLOSE) GetProcAddress(ghDll, fnSiClose)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiGetEventWinInit = (PFNSI_GETEVENTWININIT) GetProcAddress(ghDll, fnSiGetEventWinInit)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiIsSpaceWareEvent = (PFNSI_ISSPACEWAREEVENT) GetProcAddress(ghDll, fnSiIsSpaceWareEvent)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiGetEvent = (PFNSI_GETEVENT) GetProcAddress(ghDll, fnSiGetEvent)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiRezero = (PFNSI_REZERO) GetProcAddress(ghDll, fnSiRezero)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiGrabDevice = (PFNSI_GRABDEVICE) GetProcAddress(ghDll, fnSiGrabDevice)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiReleaseDevice = (PFNSI_RELEASEDEVICE) GetProcAddress(ghDll, fnSiReleaseDevice)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiButtonPressed = (PFNSI_BUTTONPRESSED) GetProcAddress(ghDll, fnSiButtonPressed)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiButtonReleased = (PFNSI_BUTTONRELEASED) GetProcAddress(ghDll, fnSiButtonReleased)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiSetUiMode = (PFNSI_SETUIMODE) GetProcAddress(ghDll, fnSiSetUIMode)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiGetDevicePort = (PFNSI_GETDEVICEPORT) GetProcAddress(ghDll, fnSiGetDevicePort)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiGetDriverInfo = (PFNSI_GETDRIVERINFO) GetProcAddress(ghDll, fnSiGetDriverInfo)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiGetLibraryInfo = (PFNSI_GETLIBRARYINFO) GetProcAddress(ghDll, fnSiGetLibraryInfo)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSiGetDeviceInfo = (PFNSI_GETDEVICEINFO) GetProcAddress(ghDll, fnSiGetDeviceInfo)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
         
      if ((pfnSpwErrorString = (PFNSPW_ERRORSTRING) GetProcAddress(ghDll, fnSpwErrorString)) == NULL)       
         {
         nResult = FAILED;
         nErrorCode = DLL_FUNCTION_LOAD_FAILURE;
         }
...
enum SpwRetVal SiInitialize(void)
{
   enum SpwRetVal tmpRetVal;

   if (gInitStatus != LOADED)
      {
      if(__SiAppInitialize() == DLL_LOAD_FAILURE)
	     {
...
int SiGetNumDevices (void)
{
   ...
   if (gInitStatus != LOADED)
      ...
      if (gInitStatus == FAILED)
         {
         SpwErrorVal = SI_NO_DRIVER;  /* global variable */
         return -1;
         }
      }
...
enum SpwRetVal SiBeep(SiHdl hdl, char *pString)
{
   ...
   if (gInitStatus != LOADED)
      ...
      if (gInitStatus == FAILED)
         {
         return SI_BAD_HANDLE;   /* only available option */
         }
      }
...
enum SpwRetVal SiSetUiMode (SiHdl hdl, SPWuint32 mode)
{
   ...
   if (gInitStatus != LOADED)
      ...
      if (gInitStatus == FAILED)
         {
         return SPW_ERROR;  /* not defined in spec, so this is a guess */
         }
      }
