@ECHO OFF
REM
REM   This a batch file used to build the siapp static library
REM
REM   Matt Farr
REM
REM   3 February 1998
REM
REM   $Id: build.bat,v 1.2 1998/02/23 20:15:41 mfarr Exp $
...
echo Preparing to build SIAPP.LIB
...
echo Cleaning: ..\..\bin\siapp\%ARCH%\Debug
nmake /c /f "siapp.mak" CFG="siapp - Win32 %ARCH% Debug sthread" CLEAN
nmake /c /f "siapp.mak" CFG="siapp - Win32 %ARCH% Debug mthread" CLEAN
:cleanr
if "%RELEASE%" == "False" goto build
echo Cleaning: ..\..\bin\siapp\%ARCH%\Release
nmake /c /f "siapp.mak" CFG="siapp - Win32 %ARCH% Release sthread" CLEAN
nmake /c /f "siapp.mak" CFG="siapp - Win32 %ARCH% Release mthread" CLEAN
goto end
...
if not exist ../../bin/siapp mkdir "../../bin/siapp"
if not exist ../../bin/siapp/%ARCH% mkdir "../../bin/siapp/%ARCH%"
if not exist ../../bin/siapp/%ARCH%/debug mkdir "../../bin/siapp/%ARCH%/debug"
if not exist ../../bin/siapp/%ARCH%/release mkdir "../../bin/siapp/%ARCH%/release"

:build
:buildd
if "%DEBUG%" == "False" goto buildr
echo Building: %ARCHC% Debug
nmake /c /f "siapp.mak" CFG="siapp - Win32 %ARCH% Debug sthread" ALL
nmake /c /f "siapp.mak" CFG="siapp - Win32 %ARCH% Debug mthread" ALL
:buildr
if "%RELEASE%" == "False" goto end
echo Building: %ARCHC% Release
nmake /c /f "siapp.mak" CFG="siapp - Win32 %ARCH% Release sthread" ALL
nmake /c /f "siapp.mak" CFG="siapp - Win32 %ARCH% Release mthread" ALL
